...
function is_pro_updated() {
	...
	function sr_schedule_daily_summary_mails() {
        ...
        in_array( 'wp-e-commerce/wp-shopping-cart.php', get_option( 'active_plugins' ) ) || ( is_multisite() && in_array( 'wp-e-commerce/wp-shopping-cart.php', get_option( 'active_sitewide_plugins' ) ) ) ) {
	...
	function sr_console_common() {
		...
		if ( isset($_GET ['action']) && $_GET ['action'] == 'sr-settings') {
			...
			if ( SRPRO === false ) {
				...
				if ( !get_option('sr_dismiss_admin_notice') && (get_option('sr_in_app_promo') == 1 || get_option('sr_in_app_promo') !== false) ) {
					// Code to handle SR IN App Promo
					if ( is_admin() ) {

						if(!get_option('sr_dismiss_admin_notice')){
							sr_add_promo_notices();
						}
					}	
				} else {
						?>
			...
			<div class="wrap" style="height:1em;">
