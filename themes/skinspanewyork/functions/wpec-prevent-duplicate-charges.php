 ...
 * 		the purchase order immediately after the transaction has been run.
 * 
...
//Filter the text on the Transaction Results page to make it clearer that a purchase may have been made.
add_action('wpsc_get_transaction_unauthorized_view', 'wpecdup_get_transaction_unauthorized_view');

function wpecdup_get_transaction_unauthorized_view(){
  return "Oops. Did you click the purchase button twice? <br /> No worries, your purchase went through. You'll see the details in your email.";
}
...
function wpecdup_before_submit_checkout() {
	...
	if($processed_session_id) {
		...
		$r = wpsc_update_customer_meta( 'checkout_session_id', $processed_session_id );
		error_log($r);
...
function wpecdup_submit_checkout_gateway($submitted_gateway, $purchase_log) {
	...
	//Gateway is about to begin, we need to make sure when the gateway marks the purchase order as approved, that we catch it right away
...
function wpecdup_purchase_log_update($purchase_log) {
	...
	//Now let's log this immediately so we know not to charge for this order again
...
// function wpecdup_throw_error($id, $status, $old_status, $purchase_log) { // aaron - why all these params here?
// 	if($status == 3) {
