	...
	<li class="<?php if($p==$k){ echo " last"; } if($p==1){ echo " first"; } ?>">
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="img-shadow">
	    		<?php $postImage = templatemela_get_first_post_images(get_the_ID()); ?>
				<div class="main">
					...
					<?php if(get_option('tmoption_portfolio_layout') =='1'){
						templatemela_print_images_thumb($postImage, get_the_title(get_the_ID()) ,320,220,'left');
						} ?>
					<?php if(get_option('tmoption_portfolio_layout') =='2'){
						templatemela_print_images_thumb($postImage, get_the_title(get_the_ID()) ,483,360,'left');
						} ?>
					<?php if(get_option('tmoption_portfolio_layout') =='3'){
						templatemela_print_images_thumb($postImage, get_the_title(get_the_ID()) ,318,270,'left');
						} ?>
					<?php if(get_option('tmoption_portfolio_layout') =='4'){
						templatemela_print_images_thumb($postImage, get_the_title(get_the_ID()) ,234,200,'left');
					} ?>
					</a>
			...
			<?php  if(get_option('tmoption_portfolio_layout') =='1'){echo templatemela_excerpt(160);} ?>
			<?php  if(get_option('tmoption_portfolio_layout') =='2'){echo templatemela_excerpt(40);} ?>
			<?php  if(get_option('tmoption_portfolio_layout') =='3'){echo templatemela_excerpt(30);} ?>
			<?php  if(get_option('tmoption_portfolio_layout') =='4'){echo templatemela_excerpt(20);} ?>
		</div>
	</li>
...
<?php if ( $wp_query->max_num_pages > 1 ) : ?>
	<div id="nav-below" class="navigation">
		<?php templatemela_get_pagination(); ?>
	</div><!-- #nav-below -->
	<?php 
	$wp_query = null;
	$wp_query = $temp;
...
<?php foreach ($categories as $category_item ) { ?>
	...
	<?php while (have_posts()) : the_post(); ?>
		<?php $image = templatemela_get_first_post_images(get_the_ID()); ?>		
		<div class="<?php echo $category_item->cat_name ?> text-box main">
			<a href="<?php echo $image; ?>" title="Click to view Full Image" data-lightbox="example-set">
				<?php templatemela_print_images_thumb($image, get_the_title(get_the_ID()) ,260,260,'left'); ?>	
				<div class="image-bottom">
<div class="image-bottom_inner"></div>
</div>
			</a>					
